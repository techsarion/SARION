...
/**
 * Inline brand SVGs. lucide-react (the project icon library) ships no brand
 * marks, so these are hand-authored, optically-sized 24×24 paths using
 * `currentColor` — zero extra dependencies, fully tree-shaken, and theme-aware.
 */
...
const LINKS = [
  ...
  { key: "x", label: "X (formerly Twitter)", href: siteConfig.social.x, Icon: XIcon },
  { key: "instagram", label: "Instagram", href: siteConfig.social.instagram, Icon: InstagramIcon },
] as const;

interface SocialLinksProps {
  /** Optional accessible label for the wrapping nav. */
  ariaLabel?: string;
  className?: string;
}

/**
 * Reusable row of official social-profile links. Single source of truth for
 * every placement (footer, contact page, …) so icons and URLs never drift.
 */
export function SocialLinks({ ariaLabel = "Social media", className }: SocialLinksProps) {
  return (
    <nav
